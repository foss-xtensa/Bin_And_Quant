...
def main(_):
  ...
  import numpy as np
  #bin and quant implementation
  print(FLAGS.var_to_compress)
  print( "bins", FLAGS.bins)
  
  v1 = sess.graph.get_tensor_by_name(FLAGS.var_to_compress)
  #print("final_fc_weights:", v1)
  v2_r = sess.run(v1)
  v2_d1 = np.shape(v2_r)[0]
  v2_d2 = np.shape(v2_r)[1]    
  v2 = np.reshape(v2_r,(v2_d1*v2_d2,1))
  v2_max = v2_r.max()
  v2_min = v2_r.min()

  #RangeValues = [v2_min, np.mean(v2) - np.std(v2), 0, np.mean(v2) + np.std(v2), v2_max]
  ...
  if FLAGS.bins == '2':
    ...
    RangeValues = [v2_min, (RangeValues[0] + RangeValues[1]) /2, np.mean(v2) - np.std(v2), 0, np.mean(v2) + np.std(v2), \
