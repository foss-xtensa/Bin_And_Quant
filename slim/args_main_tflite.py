...
def test_model_accuracy(tflite_model, model_name):
    command = "python3 ./tflite_eval.py --alsologtostderr  --dataset_dir=../../MobileNet/models/research/imagenet-data     --dataset_name=imagenet     --dataset_split_name=validation     --model_name=" + args.model_name + " --batch_size=10 --tflite_file=" + args.tflite_model +" --eval_size=" +eval_size
...
modified_file = args.model_name + '_modified.tflite'
top_model_file = args.model_name'_top_acc_model.tflite'
...
num_layers = args.total_layers #starts with zero
curr_layer = 0


if curr_layer >0:
    top_model_file = args.tflite_model
...
for curr_layer in range(num_layers): ########## change range from 0,num_layers
    ...
    top_acc = curr_acc
    model = load_model_from_file(tflite_model)
    top_model_file = args.model_name + '_top_acc_model_' +str(curr_layer)+'_.tflite'
