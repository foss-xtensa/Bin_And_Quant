...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      signature = predict_signature_def(inputs={'input': x}, outputs={'MobilenetV2/Predictions/Reshape_1': y})
     ...
     '''
      ...
      with tf.Session() as sess:
          with slim.queues.QueueRunners(sess):
            #for i in range(4):
                images, np_label = sess.run([image, label])
          import pdb
          pdb.set_trace()
          converter = tf.lite.TFLiteConverter.from_frozen_graph("tensorflow/frozen_mobilenet_v2_test.pb", input_arrays=["input"], output_arrays=["MobilenetV2/Predictions/Reshape_1"])
          #float_tflite_model = float_converter.convert()
          #float_tflite_model_size = open(FLOAT_MODEL_TFLITE, "wb").write("float_tflite_mobilenet.tflite")
          ...
          #flattened_data = np.array(images[0].flatten(), dtype=np.float32).reshape(1, 150528)
          #converter.representative_dataset = flattened_data
